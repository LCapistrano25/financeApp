...
export function BottomSheet({ isOpen, onClose, title }: BottomSheetProps) {
  ...
  return (
    <>
      ...
      {/* A GAVETA */}
      <div 
        className={`fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 rounded-t-[24px] p-6 z-50 transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? "translate-y-0" : "translate-y-full"}`}
      >
        {/* TRACINHO SUPERIOR (Handle) */}
        <div className="w-10 h-1.5 bg-gray-200 dark:bg-slate-700 rounded-full mx-auto mb-6" />
